...
class APIdb {
  ...
  /**
   * Call API and replace all data in memory with what returns. Also establishes socket event.
   * 
   * @param {APIreq} APIreq The request to use
   * @param {string} path URL path relative to the server
   * @param {object} data Data to pass in the request
   */
  replaceData (APIreq, path, data = {}) {
    try {
      APIreq.request({ method: 'POST', url: path, data: data }, (body) => {
  ...
  /**
   * Assign a socket event to this database which follows websocket standards for data.
   * 
...
// Class for managing requests and authorization to API
// eslint-disable-next-line no-unused-vars
...
class APIreq {
   ...
   * Construct the class
   * 
   * @param {sails.io} socket socket connection
   * @param {string} host Host name of this client
   * @param {?APIdb} db Reference to the APIdb containing records of those that can authorize with this request, if applicable
   * @param {?object} filter Filter applicable records that can authorize in the APIdb by this TAFFY query object, if applicable
   * @param {?string} usernameField Name of the database column containing names for authorization, if applicable
   * @param {?string} authPath URL path in API for authorization and getting a token, if applicable
   ...
   */
  constructor(socket, host, db = null, filter = null, usernameField = null, authPath = null, authName = null) {
    this.socket = socket
    this.host = host
    this.db = db
    this.filter = filter
  ...
  /**
   * Check authorization, and then make a request to API
   * 
  ...
  /**
   * Make an authorization request for a token, and store the token in memory.
   * 
  ...
  getUrlParameter (name) {
    try {
      ...
      $(document).Toasts('create', {
        ...
        body: 'There was an error in the getUrlParameter function.',
        icon: 'fas fa-skull-crossbones fa-lg',
      });
    }
  }
  ...
  hexRgb (hex, options = {}) {
    ...
    try {
      ...
      $(document).Toasts('create', {
        class: 'bg-danger',
        title: 'hexrgb error',
        body: 'There was an error in the hexrgb function.',
        icon: 'fas fa-skull-crossbones fa-lg',
      });
    }
  }
...
if (typeof require !== 'undefined') {
  ...
  exports.APIScriptLoader = APIScriptLoader;
  exports.APIutil = APIutil;
  exports.APIqueue = APIqueue;
  exports.APImodal = APImodal;
  exports.APIanimations = APIanimations;
}