  ...
  fn: async function (inputs) {
    ...
    if (currentScore < 100 && newScore >= 100) {
      ...
      if (inputs.member.settings.spamScoreStamp === null || moment().subtract(1, 'minutes').isAfter(moment(inputs.member.settings.spamScoreStamp))) {
        if (inputs.message) {
          var response = `:warning: <@${inputs.message.author.id}> **Hey kid, stop the spamming.** Rest from sending messages for the next ${moment.duration(inputs.member.guild.settings.antispamCooldown > 0 ? (newScore / inputs.member.guild.settings.antispamCooldown) + 1 : 0, 'minutes').format("m [Minutes]")}. Otherwise, you're gonna have a bad time. `;
