...
module.exports = {
  ...
  fn: async function (inputs) {
    ...
    if (
      typeof sails.helpers.tasks === "undefined" ||
      typeof sails.helpers.tasks[inputs.record.task] === "undefined"
    )
      return;
    ...
    if (inputs.record.nextRun && !inputs.record.cron) {
      var temp = (async (record) => {
        Schedules[record.id] = new CronJob(
      ...
      if (
        inputs.record.nextRun &&
        moment().isBefore(moment(inputs.record.nextRun))
      ) {
          ...
          Schedules[record.id] = new CronJob(
            ...
            async () => {
              ...
              Schedules[record.id] = new CronJob(
                record.cron,
                async () => {
                  await sails.helpers.tasks[record.task].with(
                    record.data || {}
                  );

                  // Update lastRun
                  await sails.models.schedules.updateOne(
                    { id: record.id },
        ...
        // no nextRun? Just schedule the cron
        var temp = (async (record) => {
          Schedules[record.id] = new CronJob(
            record.cron,
            async () => {
