module.exports = {
  ...
  description: 'Resolve a username to a Discord user, or prompt if multiple ones detected.',
...
function resolveUser (query, guild) {
  if (query instanceof Discord.GuildMember)
    return query.user;
  if (query instanceof Discord.User)
    return query;
  if (typeof query === 'string') {
    if (sails.config.custom.discord.regex.userOrMember.test(query))
      return guild.client.users.fetch(sails.config.custom.discord.regex.userOrMember.exec(query)[ 1 ]).catch(() => null);
