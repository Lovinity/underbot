module.exports = {
  ...
  description: 'Resolve a role name to a Discord role, or prompt if multiple roles match criteria.',
...
function resolveRole (query, guild) {
  ...
  if (typeof query === 'string' && sails.config.custom.discord.regex.role.test(query))
    return guild.roles.resolve(sails.config.custom.discord.regex.role.exec(query)[ 1 ]);
  return null;
}
