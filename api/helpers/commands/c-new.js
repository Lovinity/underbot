...
module.exports = {
  ...
  fn: async function (inputs) {
    ...
    var name = await prompt(
      `Let's get started with adding a new character into the database! First, what is the name of the character? You will use this name in character commands. (timeout: 2 minutes)`,
      120000
    );
    ...
    if (prevCharacter) {
      ...
      await prompt(
        `:warning: A character with that name already exists. If you proceed, you will be overwriting the character sheet. Type/send "cancel" to exit, or send anything else to proceed. (timeout: 2 minutes)`,
        120000
      );
    }
    ...
    var nicknames = await prompt(
      `Killer name! What other (nick) names does this character get called? (timeout: 2 minutes)`,
      120000
    );
    ...
    var OC = await prompt(
      `Aww, those are cute! Is this character an OC character (NOT an original Undertale character)? (timeout: 2 minutes)`,
      120000
    );
    OC =
    ...
    var claimable = await prompt(
      `Cool! Can this character be re-claimed by someone else should the original claimer leave the guild for more than 24 hours? (if no, the character would be deleted instead) (timeout: 2 minutes)`,
      120000
    );
    claimable =
    ...
    var owner = await prompt(
      `Alrighty! Now, Please provide the username, snowflake ID, or mention of the member who owns / has claim over this character. Or, type "none" if this character is not claimed yet. (timeout: 2 minutes)`,
      120000
    );
    if (owner.cleanContent.toLowerCase() === "none") {
      owner = null;
    } else {
      owner = await sails.helpers.resolvers.username(
        inputs.message,
        owner.content
      );
      owner = owner.id;
    ...
    var pronouns = await prompt(
      `Aww how adorable! Now, what pronouns does this character use (such as he/him/his, she/her/hers, or they/them/theirs) ? (timeout: 2 minutes)`,
      120000
    );
    ...
    var age = await prompt(
      `Sweet! How old is this character? (timeout: 2 minutes)`,
      120000
    );
    ...
    var height = await prompt(
      `They say age is your real-life level. How tall is this character? (timeout: 2 minutes)`,
      120000
    );
    ...
    var appearance = await prompt(
      `They should play basketball! Provide any text descriptions of the character's appearance. Or, type "none" to leave this blank. (timeout: 10 minutes)`,
      600000
    );
    ...
    var personality = await prompt(
      `Wicked neat! Describe this character's personality. Or, type "none" to leave this blank. (timeout: 10 minutes)`,
      600000
    );
    ...
    var likes = await prompt(
      `Haha sounds like the perfect friend! Make a list of things this character likes, or type "none". (timeout: 5 minutes)`,
      300000
    );
    ...
    var dislikes = await prompt(
      `Kawaii! Make a list of things this character dislikes, or type "none". (timeout: 5 minutes)`,
      300000
    );
    ...
    var soulType = await prompt(
      `Ooh, we might be enemies then! What soul type does this character have? Or, type "none" to leave this blank. (timeout: 2 minutes)`,
      120000
    );
    ...
    var maxHP = await prompt(
      `oooh! If this character has a static max HP that does not change, provide it here. Or, type 0 if this character's static HP should depend on their LVL (standard human Undertale EXP system). (Must be a number, eg 20) (timeout: 2 minutes)`,
      120000
    );
    ...
    // Prompt for DT
    var DT = await prompt(
      `Sweet! Specify the character's starting DT (determination). (Must be a number, eg 20, and must be between 0 and 100.) (timeout: 2 minutes)`,
      120000
    );
    ...
    var EXP = await prompt(
      `Specify this character's starting EXP. (Must be a number, eg 20) (timeout: 2 minutes)`,
      120000
    );
    ...
    var ATK = await prompt(
      `What is this character's ATK? You should also provide in parenthesis how much of an effect, in HP, the character's ATK has. For example: "10 (2)" (timeout: 2 minutes)`,
      120000
    );
    ...
    var DEF = await prompt(
      `What is this character's DEF? You should also provide in parenthesis how much of an effect, in HP, the character's DEF has. For example: "10 (2)" (timeout: 2 minutes)`,
      120000
    );
    ...
    var extraInfo = await prompt(
      `Sounds like a real Knight! Finally, provide any additional information about the character not already covered, or type "none". (timeout: 10 minutes)`,
      600000
    );
