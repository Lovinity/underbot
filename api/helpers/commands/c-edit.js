...
module.exports = {
  ...
  description: 'Edit a stat or information about a character (items has its own command). (Staff-only command).',
  ...
  inputs: {
    ...
    character: {
      type: 'string',
      required: true,
      description: 'The name of the character in the database to edit something.'
    },
  },
  ...
  fn: async function (inputs) {
    ...
    switch (property) {
      ...
      case 'sprite':
        ...
        break;
      case 'pronouns':
        var pronouns = await prompt(`What pronouns does this character use? Current value is **${character.pronouns}** (timeout: 2 minutes)`, 120000);
        toUpdate.pronouns = pronouns.cleanContent;
        break;
      case 'age':
        var age = await prompt(`How old is this character? Current value is **${character.age}** (timeout: 2 minutes)`, 120000);
        toUpdate.age = age.cleanContent;
        break;
      case 'height':
        var height = await prompt(`How tall is this character? Current value is **${character.height}** (timeout: 2 minutes)`, 120000);
        toUpdate.height = height.cleanContent;
        break;
      case 'appearance':
        var msg = await inputs.message.send("```" + character.appearance + "```");
        var appearance = await prompt(`Provide text descriptions of the character's new appearance. Or, type "none" to leave this blank. (timeout: 10 minutes)`, 600000);
        ...
        break;
      case 'personality':
        var msg = await inputs.message.send("```" + character.personality + "```");
        var personality = await prompt(`Describe this character's new personality. Or, type "none" to leave this blank. (timeout: 10 minutes)`, 600000);
      ...
      case 'likes':
        ...
        var likes = await prompt(`Make a new list of things this character likes, or type "none". (timeout: 5 minutes)`, 300000);
      ...
      case 'dislikes':
        ...
        var dislikes = await prompt(`Make a new list of things this character dislikes, or type "none". (timeout: 5 minutes)`, 300000);
        ...
        break;
      case 'soultype':
        var soulType = await prompt(`What soul type does this character have? Or, type "none" to leave this blank. Current value: **${character.soulType}** (timeout: 2 minutes)`, 120000);
        ...
        break;
      case 'hp':
        var HP = await prompt(`Provide the character's current HP. (Must be a number, eg 20) (timeout: 2 minutes)`, 120000);
      ...
      case 'exp':
        ...
        break;
      case 'atk':
        var ATK = await prompt(`What is this character's new ATK? You should also provide in parenthesis how much of an effect, in HP, the character's ATK has. For example: "10 (2)" (timeout: 2 minutes)`, 120000);
      ...
      case 'armor':
        ...
        break;
      case 'extrainfo':
        var msg = await inputs.message.send("```" + character.extraInfo + "```");
        var extraInfo = await prompt(`Provide any additional information about the character not covered by other fields, or type "none". (timeout: 10 minutes)`, 600000);
