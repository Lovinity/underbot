  ...
  fn: async function (inputs) {
    ...
    confirmSchedule = async () => {
      var confirm = await prompt(
        ...
        )}. Is this correct? (timeout: 1 minute)`,
        60000
      );
      if (
        confirm.cleanContent.toLowerCase() !== "yes" &&
        confirm.cleanContent.toLowerCase() !== "y"
      ) {
    ...
    rePromptSchedule = async () => {
      datetime = await prompt(
        `Please specify a different date/time for your reminder. Specify your timezone if different from Eastern Time (America/New_York). (timeout: 2 minutes)`,
        120000
      );
