module.exports = {
  ...
  description:
    "Generate a time-ordered text file of messages from the RP channels.",
      ...
      return new Promise(async (resolve, reject) => {
        setTimeout(async () => {
          ...
          msgs.map((message) => {
            ...
            data += `+++Message in channel ${
              message.channel.parent ? message.channel.parent.name : `Unknown`
            } => ${message.channel.name} by ${message.author.username}#${
              message.author.discriminator
            } (${message.author.id}), ID ${message.id}+++\n`;
          ...
          await inputs.message.send({
            files: [
              {
                attachment: buffer,
                name: `RPtext_${await sails.helpers.uid()}.txt`,
              },
            ],
          });
