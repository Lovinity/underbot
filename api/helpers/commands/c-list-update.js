module.exports = {
    ...
    fn: async function (inputs) {
        ...
        if (inputs.message.guild.settings.ocChannel) {
            var channel = await DiscordClient.channels.resolve(inputs.message.guild.settings.ocChannel);
            if (channel) {
        ...
        if (inputs.message.guild.settings.ogChannel) {
            var channel = await DiscordClient.channels.resolve(inputs.message.guild.settings.ogChannel);
            if (channel) {
                var maps2 = inputs.message.guild.characters
